            ...
            if isinstance(callbacks, list) and all(isinstance(cb, str) for cb in callbacks):
                for callback_name in callbacks:
                    match callback_name:
                        ...
                        case "federated":
                            cbs.append(FedWeightUpdateCallback(self.config.update_freq, self.config.result_queue, self.config.update_queue, verbose=self.config.verbose))
