...
async def test_fed(feed_name, seed, result_queue, update_queue, update_freq, isMain, port):
    ...
    try:
        ...
        agent = FedAgent(
            config=FedConfig(
                mode="train",
                # model_file="fedModels/Agents=3_Configuration=Default + Seq + tc30 + final (batch 512) (lin 0.0028) (ent 0.1)/last_default.zip",
                deterministic=False,
                model_name="sac",
                episodes=episodes,
        ...
        if isMain:
            network_controller = NetworkController(gt_bandwidth=15.0, interval=30.0, interface="lo")
