...
def create_workers(num_workers, result_queue, update_queue, update_freq):
    ...
    for i in range(num_workers):
        ...
        if i == 0:
            ...
            p.start()
            workers.append(p)
    p = Process(target=gcc_stream_start, args=(PORT_START + num_workers,))
    p.start()
    workers.append(p)   
    return workers
...
async def test_fed(feed_name, seed, result_queue, update_queue, update_freq, isMain, port):
    ...
    try:
        ...
        agent = FedAgent(
            config=FedConfig(
                mode="eval",
                model_file="fedModelsReward/rewardRate = 0.10, bandwidth=30.0, jitter = 0.3, extra_gcc/drl_model_5000_steps.zip",
                deterministic=True,
                model_name="sac",
                episodes=episodes,
        ...
        if isMain:
            network_controller = NetworkController(gt_bandwidth=30.0, interval=15.0, interface="lo")
...
async def gcc_stream(port):
    try:
        mqtt_cfg = MqttConfig(feed_name="extra", broker_port=1884)
